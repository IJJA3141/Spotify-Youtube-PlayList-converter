...
async function getTokens(obj, prom){
    ...
    body += "&redirect_uri=" + encodeURI(redirectUri + "/loginresponse/" + obj.name);
    body += "&client_id=" + obj.clientId;
    body += "&client_secret=" + obj.clientSecret;

    callAuthorizationApi(body).then(response => {if (response.status == 200){
        let data = JSON.parse(response.responseText)
        if (data.access_token != undefined){
            obj.token = data.access_token
            console.log(obj)
        } else {
            console.log("probleme responce token" + response.responseText)
            process.exit()
        }
    }})
}
